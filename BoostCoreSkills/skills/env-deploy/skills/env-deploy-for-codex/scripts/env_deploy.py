#!/usr/bin/env python3
"""Automated Linux environment deployment helper for the env-deploy-for-codex Skill.
...
def write_setup(ctx: DeployContext) -> None:
    lines = [
        "#!/usr/bin/env bash",
        "# Generated by env-deploy-for-codex. Review before running on the target host.",
...
def resolve_source(source: str | None, clone_dir: Path | None, apply: bool) -> Path:
    ...
    if re.match(r"^(git@|https?://)", source):
        target_root = (clone_dir or (Path.cwd() / "env-deploy-for-codex-sources")).expanduser().resolve()
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Automate env-deploy-for-codex discovery, execution, logging, and artifact generation.")
...
def print_human_summary(summary: dict[str, object]) -> None:
    print("env-deploy-for-codex summary")
